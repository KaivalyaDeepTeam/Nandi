"""
MT5 File Bridge Connector.

Communicates with the MetaTrader 5 Expert Advisor via shared CSV files.
The EA writes market data to files, and reads trade commands from files.
"""
...
from config.settings import MT5_FILES_DIR
...
class MT5Connector:
    ...
    def __init__(self, files_dir: str = MT5_FILES_DIR):
    ...
    def connect(self) -> bool:
        if not os.path.isdir(self.files_dir):
            ...
            logger.error("Make sure MT5 is running with the ForexPredictor EA attached.")
            return False

        # Check if EA is writing files
        if os.path.exists(self.tick_file):
            self.connected = True
            logger.info(f"Connected to MT5 via files at {self.files_dir}")
            return True
        else:
            logger.error("EA files not found. Make sure ForexPredictor EA is running on a chart.")
            return False
    ...
    def _wait_for_response(self, timeout: float = 30.0) -> str:
        """Wait for the EA to write a response file."""
        start = time.time()
    ...
    def _send_command(self, *args) -> str:
        """Write a command file for the EA to process."""
    ...
    def open_trade(self, symbol: str, order_type: str, lot_size: float,
                   sl_price: float = 0, tp_price: float = 0, comment: str = "") -> dict:
    ...
    def modify_trade(self, ticket: int, sl_price: float = 0, tp_price: float = 0) -> dict:
    ...
    def get_symbol_info(self, symbol: str) -> dict:
        # Symbol info is included in the tick file
