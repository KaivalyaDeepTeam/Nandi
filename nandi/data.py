"""
Nandi Data Pipeline — Download 20+ years of forex data and compute features.
"""
...
from nandi.config import DATA_DIR, SYMBOL, LOOKBACK_YEARS, TEST_MONTHS
...
def download_forex_data(symbol="eurusd", years=LOOKBACK_YEARS):
    ...
    # Add volume column (0 for forex — not available from Stooq)
...
def compute_features(df):
    """Compute 40+ features for the RL agent."""
    ...
    for period in [14, 28]:
        ...
        rs = gain / (loss + 1e-10)
        f[f"rsi_{period}"] = (100 - 100 / (1 + rs) - 50) / 50  # normalized to [-1, 1]
...
def prepare_data(lookback_window=60, test_months=TEST_MONTHS, years=LOOKBACK_YEARS):
    """Full pipeline: download → features → normalize → split."""
    from sklearn.preprocessing import RobustScaler

    df = download_forex_data(years=years)
    ...
    test_start = features.index[-1] - pd.DateOffset(months=test_months)
    ...
    logger.info(f"Train: {n_train} days ({features.index[train_mask][0].date()} → "
                f"{features.index[train_mask][-1].date()})")
    logger.info(f"Test:  {n_test} days ({features.index[test_mask][0].date()} → "
                f"{features.index[test_mask][-1].date()}) [UNSEEN]")
    ...
    return {
        ...
        "train_prices": df[train_mask]["close"].values,
        "test_prices": df[test_mask]["close"].values,
